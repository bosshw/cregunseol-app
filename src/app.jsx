...
const DB = {
  ...
  recordSale(id, opt) {
    ...
    if (idx >= 0) {
      ...
      evs[idx] = { ...cur, date: d, data: { ...(cur.data || {}), price: won ? String(won) : '', free, notes } };
      this.saveEvents(evs);
    } else {
      this.addEvent({ individualId: id, type: 'distribution', date: d,
        data: { price: won ? String(won) : '', free, notes: buyer ? '분양처: ' + buyer : '' } });
    }
    return { won, free, buyer, date: d };
  },

  /* ── 알 하나하나의 기록 ──
     산란 한 건 안에 알이 여러 개일 때, 알마다 겉보기 상태와 결과를 따로 남깁니다. */
  setEggUnits(layingId, units) {
    const ev = this.getEvents().find(e => e.id === layingId);
    if (!ev) return null;
    const clean = (units || []).map(u => ({
      look: u.look || '', status: u.status || 'pending',
      reason: u.reason || '', note: u.note || '', date: u.date || '',
    }));
    const data = { ...(ev.data || {}), eggUnits: clean };
    // 알이 전부 무정란이면 예전 방식(메모의 '무정')과도 맞춰 둡니다 — 옛 화면에서도 같게 보이도록
    const allInf = clean.length > 0 && clean.every(u => u.status === 'infertile');
    const notes = String(data.notes || '');
    if (allInf && !/무정/.test(notes)) data.notes = (notes ? notes + ' · ' : '') + '무정';
    if (!allInf && /무정/.test(notes)) {
      data.notes = notes.split('·').map(x => x.trim()).filter(x => x && !/^무정/.test(x)).join(' · ');
    }
    this.updateEvent(layingId, { data });
    return clean;
  },
...
/* ══════════════════════════════════════════
   ...
   ══════════════════════════════════════════ */
/* ★ 화면에 보이는 버전은 대표님이 정합니다. 숫자가 올라가기만 하는 값이 아닙니다.
   2026-09-10 에 1.7 → 1.0 으로 되돌렸습니다(그 전에도 5.4 → 1.0 이 있었습니다).
...
function contactLinks(settings) {
  const mine = (settings || {}).contact || {};
  return CONTACT_FIELDS.map(([k, label]) => {
    let url = (CONTACT[k] || mine[k] || '').trim();
    if (url && !/^https?:\/\//i.test(url)) url = 'https://' + url;
    return { key: k, label, url };
  }).filter(x => x.url);
}
...
/* 분양완료인데 분양가를 아직 안 적은 아이들 — 무료로 보낸 아이는 뺍니다 */
function soldMissingPrice(individuals) {
  const list = individuals || DB.getIndividuals();
  return list.filter(i => (i.status || 'own') === 'sold' && !i.isExternal
    && !salePriceOf(i) && !saleIsFree(i));
}
/* 무료로 보낸 아이인지 — 개체 표시 → 분양 기록 순으로 봅니다 */
function saleIsFree(ind) {
  if (!ind) return false;
  if (ind.saleFree) return true;
  return DB.getEventsFor(ind.id).some(e => e.type === 'distribution' && e.data && e.data.free
    && !Number(e.data.price || 0));
}
/* 화면에 보여줄 분양가 글자 — 값이 있으면 금액, 무료면 '무료', 아직 안 적었으면 빈 글자 */
function salePriceLabel(ind) {
  const w = salePriceOf(ind);
  if (w > 0) return wonText(w);
  return saleIsFree(ind) ? '무료' : '';
}
...
/* ── 알 하나하나(클러치 안의 개별 알) ──
   겉보기 상태는 산란 직후 불빛을 비춰 보고 적습니다. 결과는 알마다 따로 남습니다. */
const EGG_LOOKS = ['정상', '크기 작음', '껍질 얇음', '금 갔음', '함몰', '곰팡이'];
const EGG_PROBLEMS = ['곰팡이', '함몰·건조', '깨짐', '중도 폐사', '기타'];
const EGG_STATE = {
  pending:   { label: '대기',   color: 'var(--accent2)' },
  hatched:   { label: '부화',   color: '#2E7D46' },
  infertile: { label: '무정란', color: '#8A8177' },
  problem:   { label: '문제',   color: '#C87F2F' },
};

/* 산란 기록 하나를 알 개수만큼의 줄로 펴 줍니다.
   ★ 저장된 게 없으면 그 자리에서 만들어 냅니다(예전 기록도 그대로 보이게):
...
/* ── 가계부 ──
   ...
   지출과 분양 외 수입만 직접 적습니다. */
const EXPENSE_CATS = ['사료·먹이', '용품', '개체 구입', '전기·난방', '택배·포장', '병원', '기타'];
const INCOME_CATS = ['분양', '용품 판매', '기타'];

/* 가계부에 올릴 줄 만들기 —
   ★ 분양 수입은 분양 기록에서 그대로 가져옵니다. 따로 또 적지 않습니다(두 번 적히지 않게).
   분양가를 아직 안 적은 건은 넣지 않고, 무료 분양은 0원 수입으로 남깁니다. */
function ledgerRows(events, individuals) {
  ...
  evs.forEach(e => {
    if (e.type === 'distribution') {
      const won = Number((e.data && e.data.price) || 0);
      const free = !!(e.data && e.data.free);
      if (!won && !free) return;
      const nm = (byId[e.individualId] || {}).name || '개체';
      rows.push({ id: 'S:' + e.id, date: e.date, flow: 'in', category: '분양', amount: won,
                  label: nm + ' 분양' + (free ? ' (무료)' : ''), auto: true,
                  indId: e.individualId, ref: e.id });
    } else if (e.type === 'memo') {
      /* 엑셀에서 가져온 "입양가 30만원" 메모 = 그 아이를 데려오는 데 쓴 돈.
         날짜가 없어서(시트에 입양일이 없었음) '이전 기록'으로 따로 묶습니다 —
         월별·연별 칸은 흔들지 않고, 전체 합계와 개체 손익에는 들어갑니다. */
      const n0 = (e.data && e.data.notes) || '';
      const mm = n0.match(/입양가\s*([\d.]+)\s*만\s*원/);
      if (!mm) return;
      const won2 = Math.round(parseFloat(mm[1]) * 10000);
      if (!won2) return;
      ...
      const d = e.data || {};
      rows.push({ id: e.id, date: e.date, flow: d.flow === 'in' ? 'in' : 'out',
                  category: d.category || '기타', amount: Number(d.amount || 0),
                  label: d.notes || '', auto: false, indId: d.indId || null, ref: e.id });
    }
  });
...
/* ══ 대화로 가계부 적기·물어보기 ══
   돈 이야기는 개체가 아니라 가계부로 갑니다.
   ⚠️ 분양 이야기는 여기서 잡지 않습니다 — 분양가를 적으면 수입이 자동으로 생기므로 두 번 적히면 안 됩니다. */
const LEDGER_OUT = /샀|구입|구매|주문|결제|지출|썼어|썼다|썼고|쓴\s*돈|냈어|냈다|납부|지불|들었어|나갔어|데려왔|들여왔|영입|입양했/;
const LEDGER_IN  = /팔았|팔림|판매|입금|받았어|벌었|수입\s*(생겼|났|들어)|들어왔어/;
const LEDGER_CAT = [
...
function extractLedger(text) {
  const t = String(text || '');
  if (/분양/.test(t) && !/분양\s*받|분양\s*샵|분양가/.test(t)) return null;   // 분양 수입은 자동 생성
  const isOut = LEDGER_OUT.test(t), isIn = LEDGER_IN.test(t);
  if (!isOut && !isIn) return null;
  // 금액 — "3만원" / "30000원" / "3만"
  let won = 0;
  let m = t.match(/(\d+(?:\.\d+)?)\s*만\s*원?/);
  if (m) won = Math.round(parseFloat(m[1]) * 10000);
  else if ((m = t.match(/(\d[\d,]*)\s*원/))) won = parseInt(m[1].replace(/,/g, ''), 10);
  if (!won || !isFinite(won)) return null;
  const flow = isOut && !isIn ? 'out' : isIn && !isOut ? 'in' : 'out';
  let category = flow === 'in' ? '기타' : '기타';
  if (flow === 'out') {
    const hit = LEDGER_CAT.find(([re]) => re.test(t));
    if (hit) category = hit[1];
  } else if (/용품|물건|중고/.test(t)) category = '용품 판매';
  const notes = t.replace(/\s+/g, ' ').trim().slice(0, 40);
  return { date: explicitDate(t) || todayStr(), data: { flow, category, amount: won, notes, indId: null } };
}

/* "이번 달 얼마 썼어?" 같은 물음에 답합니다 */
function answerLedgerQuery(text) {
  const t = String(text || '');
  const money = /얼마|수입|지출|순익|수익|가계부|벌었|썼|쓴\s*돈|남았/.test(t);
  if (!money) return null;
  const asks = /얼마|알려\s*줘|보여\s*줘|알려줄래|보여줄래|되지|돼|\?|？/.test(t) || /가계부/.test(t);
  if (!asks) return null;
  const rows = ledgerRows();
  if (!rows.length) return '아직 가계부에 적힌 게 없어요 💰\n"사료 3만원 샀어"처럼 말씀하시면 바로 담아둘게요.';
  const live = rows.filter(r => !r.opening);   // 날짜가 없는 이전 기록(입양가)은 달·해 계산에서 뺍니다
  const now = todayStr();
  const thisMonth = monthKeyOf(now), thisYear = yearKeyOf(now);
  const d = new Date(now); d.setMonth(d.getMonth() - 1);
  const lastMonth = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
  let label, sel;
  if (/전체|총|누적|지금까지|다\s*합|통틀어/.test(t)) { label = '전체 기간'; sel = rows; }
  else if (/작년|지난해/.test(t)) { const y = String(Number(thisYear) - 1); label = `${y}년`; sel = live.filter(r => yearKeyOf(r.date) === y); }
  else if (/올해|금년|올\s*한\s*해/.test(t)) { label = `${thisYear}년`; sel = live.filter(r => yearKeyOf(r.date) === thisYear); }
  else if (/지난\s*달|저번\s*달|전달/.test(t)) { label = `${Number(lastMonth.slice(5))}월`; sel = live.filter(r => monthKeyOf(r.date) === lastMonth); }
  else { label = `${Number(thisMonth.slice(5))}월`; sel = live.filter(r => monthKeyOf(r.date) === thisMonth); }
  const g = ledgerSum(sel);
  if (!g.count) return `${label}에는 오간 돈이 없어요 💰`;
  const only = /지출|썼|쓴\s*돈|나갔/.test(t) && !/수입|벌|순익|수익/.test(t) ? 'out'
             : /수입|벌|들어왔/.test(t) && !/지출|썼|쓴\s*돈/.test(t) ? 'in'
             : /순익|수익|남았|남은/.test(t) ? 'net' : 'all';
  if (only === 'out') {
    const top = {};
    sel.filter(r => r.flow === 'out').forEach(r => { top[r.category] = (top[r.category] || 0) + r.amount; });
    const list = Object.entries(top).sort((a, b) => b[1] - a[1]).slice(0, 3)
      .map(([k, v]) => `${k} ${v.toLocaleString()}`).join(' · ');
    return `${label} 지출은 ${g.expense.toLocaleString()}원이에요 💸${list ? `\n${list}` : ''}`;
  }
  if (only === 'in') return `${label} 수입은 ${g.income.toLocaleString()}원이에요 💰`;
  if (only === 'net') return `${label} 순익은 ${g.net >= 0 ? '+' : ''}${g.net.toLocaleString()}원이에요${g.net >= 0 ? ' 🙂' : ' 😥'}\n수입 ${g.income.toLocaleString()} · 지출 ${g.expense.toLocaleString()}`;
  return `${label} 가계부예요 💰\n· 수입 ${g.income.toLocaleString()}원\n· 지출 ${g.expense.toLocaleString()}원\n· 남은 것 ${g.net >= 0 ? '+' : ''}${g.net.toLocaleString()}원`;
}
const monthKeyOf = iso => String(iso || '').slice(0, 7);
const yearKeyOf = iso => String(iso || '').slice(0, 4);
function ledgerSum(rows) {
  let income = 0, expense = 0;
  rows.forEach(r => { if (r.flow === 'in') income += r.amount; else expense += r.amount; });
  return { income, expense, net: income - expense, count: rows.length };
}
/* 이 아이에게 들어간 돈 — 엑셀에서 가져온 입양가 + 가계부에서 개체를 지정한 지출.
   판정을 한 곳(ledgerRows)에서만 하도록 그 결과를 그대로 씁니다. */
function costOfIndividual(id, events) {
  return ledgerRows(events).filter(r => r.flow === 'out' && r.indId === id)
    .reduce((a, r) => a + r.amount, 0);
}
/* 개체 손익 = 분양가 − 그 아이에게 쓴 돈. 분양 전이면 아직 계산하지 않습니다 */
function profitOfIndividual(ind, events) {
  if (!ind) return null;
  const sold = (ind.status || 'own') === 'sold';
  const cost = costOfIndividual(ind.id, events);
  const got = salePriceOf(ind);
  if (!sold && !cost) return null;
  return { sold, cost, got, net: got - cost };
}
...
const CALENDAR_TYPES = ['mating', 'laying', 'hatching', 'feeding', 'health'];
...
function clutchRows(individuals, events) {
  ...
  return lays.map(e => {
    ...
    else if (hatched && nPending === 0) { state = '부화완료'; tone = '#2E7D46'; }
    else if (hatched && nPending > 0) { state = `일부부화 (${nPending}개 대기)`; tone = '#C87F2F'; }
    else if (allDone && nProblem) { state = '문제'; tone = '#C87F2F'; }
    else if (allDone) { state = '종료'; tone = '#8A8177'; }
...
/* ── 조사 한 벌 ──
   ★ "크은이(이)군요" 처럼 괄호로 얼버무리지 않습니다.
     한국어는 받침만 보면 규칙이 정해져 있으니 앱 어디서든 이 함수들만 씁니다.
     새 문장을 쓸 때 "(은)는" 같은 걸 손으로 적지 마세요. */
const josa = (w, withJong, withoutJong) => `${w}${hasJong(w) ? withJong : withoutJong}`;
const iyeyo  = (w) => josa(w, '이에요', '예요');       // 크안이이에요 / 크나예요
const iga    = (w) => josa(w, '이', '가');             // 크안이가 / 크나가
const eunneun = (w) => josa(w, '은', '는');            // 크안이은 → 크안이는
const eulreul = (w) => josa(w, '을', '를');
const gwawa  = (w) => josa(w, '과', '와');
const euro   = (w) => josa(w, '으로', '로');           // 받침 ㄹ 은 '로' — 아래에서 따로 봅니다
const igunyo = (w) => josa(w, '이군요', '군요');       // 한울이군요 / 크은이군요
const irang  = (w) => josa(w, '이랑', '랑');
/* '으로'는 받침 ㄹ 일 때만 예외로 '로'를 씁니다 (서울로, 연필로) */
function euroWord(w) {
  const t = String(w || '').trim();
  if (!t) return t;
  const c = t.charCodeAt(t.length - 1);
  if (c < 0xAC00 || c > 0xD7A3) return t + '로';
  const jong = (c - 0xAC00) % 28;
  return t + (jong === 0 || jong === 8 ? '로' : '으로');   // 8 = ㄹ
}
...
function mateWarning(a, b, all) {
  ...
  return {
    ...r,
    text: r.level === 'danger'
      ? `⛔ ${r.label}.\n너무 가까운 사이라 메이팅은 피해 주세요.`
      : `⚠️ ${r.label}.\n가까운 사이니 붙이시려면 한 번 더 생각해 보시는 게 좋아요.`,
  };
}
...
function morphAnswer(a, b) {
  ...
  L.push('', '확률은 알 하나하나에 매번 새로 적용돼요.\n네 개 낳는다고 정확히 그 비율로 나오지는 않습니다.');
  return L.join('\n');
}
...
function mateAnswer(target) {
  ...
  if (!good.length) {
    L.push('', '지금 있는 아이들과는 전부 너무 가까운 사이예요.\n밖에서 짝을 찾으시는 게 좋겠어요.');
  } else {
    good.forEach((g, n) => {
      const bits = [];
...
/* 우리 집에서 나온 아이 표시 — 화면에는 MY 로 나옵니다.
   (저장 필드 이름 isFromCreGunseol/cgOff 는 기존 데이터 호환 때문에 그대로 둡니다) */
const isMine = (i) => !!i && !i.cgOff && (i.isFromCreGunseol === true || !!(i.sireId || i.damId));
...
function extractFacts(text) {
  ...
  if (RE_SHED.test(text) && !/부전/.test(text)) push('shed', {});
...
function memoReply(name, text) {
  ...
  if (AFFECTION_RE.test(text)) {
    return `${name}를 정말 아끼시는 마음이 느껴져요 🦎\n이 말씀은 ${name} 기록에 소중히 남겨둘게요.`;
  }
  return null;
}
...
function factLabel(f) {
  if (f.type === 'ledger') {
    const d0 = f.data || {};
    return `${d0.flow === 'in' ? '💰 수입' : '💸 지출'} · ${d0.category || '기타'}${d0.amount ? ' ' + wonText(d0.amount) : ''}`;
  }
...
function answerQuery(target, text) {
  ...
  if (asksNext && /산란|알/.test(text)) {
    ...
    if (e) {
      ...
      return `${target.name}의 다음 산란은 ${fmtDate(eta)}쯤으로 보여요 🥚 (${inDays(eta)})\n`
        + `마지막 산란 ${fmtDate(e.date)} 기준\n약 ${LAYING_INTERVAL}일 간격으로 잡은 예상이에요.`;
    }
    if (m) {
  ...
  if (asksNext && /해칭|부화/.test(text)) {
    /* 부화 대기 판정은 clutchRows 한 곳에서만 합니다.
       무정란·문제로 정리한 알까지 여기서 다시 세지 않도록(화면마다 답이 달라지지 않게). */
    const rows = clutchRows().filter(r => r.e.individualId === target.id);
    if (!rows.length) return none('산란', '🥚');
    const pending = rows.filter(r => r.waiting).sort((a, b) => a.d - b.d);
  ...
  if (/성별|암컷인지|수컷인지/.test(text)) {
    return `${target.name}의 성별은 ${euroWord(genderLabel(target.gender))} 기록돼 있어요.`;
  }
  ...
  if (/사진|이미지/.test(text)) {
    ...
    return photos ? `${target.name}의 사진은 ${photos}장 있어요 📷\n프로필에서 볼 수 있어요.`
                  : `${target.name}의 사진은 아직 없어요 📷\n대화창의 📷 버튼으로 올려주시면 기록에 붙여둘게요.`;
  }
...
function App() {
  ...
  const navigate = (name, props = {}) => {
    ...
    if (TAB_SCREENS.includes(name)) setTab(name);
  };
  ...
  useEffect(() => {
    const onPop = () => {
      ...
      setScreen(prev);
      if (TAB_SCREENS.includes(prev.name)) setTab(prev.name);
    };
...
function HomeScreen({ individuals, navigate, showToast, refreshIndividuals, view: initialView }) {
  ...
  const [view, setView] = useState(initialView === 'sale' ? 'own' : (initialView || 'own')); // own | mating | laying | hatch
  ...
  const ownList = useMemo(() => {
    let list = sortList(bySearch(activeInds), sortKey);
    if (favOnly) list = list.filter(i => i.favorite);
    if (keepOnly) list = list.filter(i => i.keep);
    return list;
  }, [activeInds, search, sortKey, favOnly, keepOnly]);
      ...
      <div className="screen">
        ...
        {individuals.length > 0 && (
          <div className="chip-row" style={{padding:'10px 16px 2px', marginTop:0}}>
            {[['own','🏠 축양'],['mating','💞 메이팅'],['laying','🥚 산란'],['hatch','🐣 해칭']].map(([k, l]) => (
...
function ClutchScreen({ layingId, navigate, showToast, refreshIndividuals }) {
  ...
  /* 알 하나하나 — clutchRows가 정한 것을 그대로 씁니다 (목록과 답이 달라지지 않게) */
  const units = row ? row.units : eggUnitsOf(ev, { hatched, infertile });
  const nProblem = units.filter(u => u.status === 'problem').length;
  const nPending = units.filter(u => u.status === 'pending').length;
  const allDone = nPending === 0;
  ...
  else if (hatched && nPending === 0) { state = '부화 완료'; tone = '#2E7D46'; }
  else if (hatched && nPending > 0) { state = `일부 부화 (${nPending}개 대기)`; tone = '#C87F2F'; }
  else if (allDone && nProblem) { state = '문제로 종료'; tone = '#C87F2F'; }
  else if (allDone) { state = '종료'; tone = '#8A8177'; }
  ...
  return (
    <div className="screen">
      ...
      <div style={{padding:16}}>
        ...
        {/* ── 알 하나하나 ──
            밖(홈·산란기록)에서는 산란 한 건이지만, 여기서는 알마다 따로 적을 수 있습니다.
        ...
        <div className="card" style={{margin:'0 0 12px'}}>
          ...
          {units.map((u, i) => {
            const st = EGG_STATE[u.status] || EGG_STATE.pending;
            const open = openEgg === i;
            const memo = [u.look, u.reason, u.note].filter(Boolean).join(' · ');
            return (
              <div key={i} style={{border:'1px solid var(--border)', borderRadius:10, padding:'9px 10px', marginBottom:6, background:'var(--bg3)'}}>
                ...
                {open && (
                  <div style={{marginTop:9}}>
                    <div style={{fontSize:11, color:'var(--text3)', marginBottom:5}}>겉보기 상태</div>
                    <div style={{display:'flex', flexWrap:'wrap', gap:5, marginBottom:9}}>
                      {EGG_LOOKS.map(L => (
                        <button key={L} className="chip-btn"
                          style={{fontSize:11.5, padding:'5px 10px', ...(u.look === L ? {background:'var(--accent-soft)', fontWeight:700} : {})}}
                          onClick={() => setUnit(i, { look: u.look === L ? '' : L })}>{L}</button>
                      ))}
                    </div>
                    <input className="input" style={{padding:'8px 10px', fontSize:12.5}} placeholder="메모 (선택)"
                      defaultValue={u.note}
                      onBlur={e => { if (e.target.value !== u.note) setUnit(i, { note: e.target.value }); }} />

                    {probFor === i ? (
                      <div style={{marginTop:9}}>
                        <div style={{fontSize:11, color:'var(--text3)', marginBottom:5}}>어떤 문제였나요?</div>
        ...
        {hatched ? (
          <div className="card" style={{margin:'0 0 12px'}}>
            <div style={{fontSize:12, fontWeight:700, color:'var(--text2)', marginBottom:8}}><HatchIcon size={14}/> 부화한 아이들</div>
          ...
          <div className="card" style={{margin:'0 0 12px'}}>
            ...
            {/* 모프 — 갓 나온 아이는 색이 안 잡혀 있으니 '아직 모르겠어요'를 눌러 비워둘 수 있습니다 */}
            <div style={{fontSize:11, color:'var(--text3)', margin:'8px 2px 4px'}}>모프</div>
            <input className="input" placeholder="예) 릴리화이트 · 할리퀸 · 50헷초초"
              value={morph} disabled={morphUnknown}
              style={morphUnknown ? {opacity:.5} : undefined}
              onChange={e => setMorph(e.target.value)} />
            <button className="btn btn-secondary btn-sm"
              style={{width:'auto', padding:'6px 12px', marginTop:6,
                      ...(morphUnknown ? {borderColor:'var(--accent)', color:'var(--accent)', fontWeight:700} : {})}}
              onClick={() => { setMorphUnknown(v => !v); setMorph(''); }}>
              {morphUnknown ? '✓ 아직 모르겠어요' : '아직 모르겠어요'}
            </button>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:8}}>
            ...
            {nPending > 0 && (probFor === -1 ? (
              <div className="card" style={{margin:0}}>
                <div style={{fontSize:12, color:'var(--text3)', marginBottom:6}}>어떤 문제였나요? <b>남은 알 {nPending}개</b>에 함께 적습니다.</div>
                <div style={{display:'flex', flexWrap:'wrap', gap:5}}>
                  {EGG_PROBLEMS.map(R => (
                    <button key={R} className="chip-btn" style={{fontSize:11.5, padding:'5px 10px'}}
                      onClick={() => setUnits(units.map(u => u.status === 'pending' ? { ...u, status: 'problem', reason: R } : u), `${euroWord(R)} 적었어요`)}>{R}</button>
                  ))}
                </div>
                <button className="btn btn-secondary btn-sm" style={{marginTop:8}} onClick={() => setProbFor(null)}>취소</button>
              </div>
            ) : (
              <button className="btn btn-secondary" onClick={() => setProbFor(-1)}>⚠️ 문제가 있었어요</button>
            ))}
            {allDone && (
              <div style={{fontSize:11.5, color:'var(--text3)', textAlign:'center'}}>
                알을 모두 정리했어요. 되돌리려면 위 알별 기록에서 눌러주세요.
              </div>
            )}
          </div>
        )}
...
function HatchView({ individuals, refreshIndividuals, showToast, navigate }) {
  ...
  if (!babies.length) {
    return (
      <div className="empty" style={{padding:'40px 20px'}}>
        <div className="empty-icon" style={{fontSize:36, lineHeight:1}}><HatchIcon size={48}/></div>
        ...
        <div className="card" key={b.id}>
          <div style={{cursor:'pointer'}} onClick={() => {
            ...
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <div style={{fontSize:15, fontWeight:700}}><HatchIcon size={18}/> {b.name} {genderEmoji(b.gender)}</div>
  ...
  return (
    <div style={{position:'fixed', inset:0, background:'rgba(30,20,15,.72)', zIndex:210, display:'flex',
      ...
      <div className="card" style={{margin:0, width:'100%', maxWidth:340, maxHeight:'88vh', overflowY:'auto'}} onClick={e => e.stopPropagation()}>
        ...
        <button className="btn btn-secondary btn-sm" style={{width:'100%', marginTop:8}} onClick={onClose}>나중에 적기</button>
      </div>
    </div>
  );
}
...
/* ══════════════════════════════════════════
   가계부 — [분양] · [수입지출]
   분양 목록은 v3.7까지 홈 칩에 있던 것을 그대로 옮겨온 것입니다.
...
function SaleSection({ individuals, navigate, showToast, refreshIndividuals }) {
  ...
  return (
    <>
      ...
      {/* 분양가를 아직 안 적은 아이들 — 무료로 보낸 아이는 여기 잡히지 않습니다 */}
      {missingPrice.length > 0 && (
        <div style={{margin:'10px 16px 0', padding:'9px 12px', borderRadius:10, background:'var(--accent-soft)',
                     border:'1px solid var(--accent-edge)', display:'flex', alignItems:'center', gap:10, fontSize:12.5}}>
          <span style={{color:'var(--accent2)', flex:1}}>분양가를 아직 안 적은 아이 <b>{missingPrice.length}마리</b></span>
          <button className="btn btn-primary btn-sm" style={{width:'auto', padding:'6px 12px'}}
...
/* ── 수입지출 ── */
function MoneySection({ individuals, navigate, showToast, refreshIndividuals }) {
  const [ver, setVer] = useState(0);
  const [mk, setMk] = useState(() => monthKeyOf(todayStr()));
  const [form, setForm] = useState(null);            // {flow, category, amount, unit, date, notes, indId}
  const [confirmDel, setConfirmDel] = useState(null);
  const [openShow, setOpenShow] = useState(false);     // 이전 기록 펼치기
  const refresh = () => { setVer(v => v + 1); refreshIndividuals && refreshIndividuals(); };
  const evs = useMemo(() => DB.getEvents(), [ver, individuals]);
  const rows = useMemo(() => ledgerRows(evs, individuals), [evs, individuals]);
  /* 엑셀에서 가져온 입양가는 날짜가 없어서 '이전 기록'으로 따로 둡니다.
     달·해 칸은 이걸 빼고 계산하고, 전체 합계에는 넣습니다. */
  const openRows = rows.filter(r => r.opening);
  const liveRows = rows.filter(r => !r.opening);
  const monthRows = liveRows.filter(r => monthKeyOf(r.date) === mk);
  const yearRows = liveRows.filter(r => yearKeyOf(r.date) === mk.slice(0, 4));
  const m = ledgerSum(monthRows), y = ledgerSum(yearRows), all = ledgerSum(rows), op = ledgerSum(openRows);
  const firstDate = liveRows.length ? liveRows[liveRows.length - 1].date : '';
  const lastDate = liveRows.length ? liveRows[0].date : '';
  const byId = {}; individuals.forEach(i => { byId[i.id] = i; });
  const shiftMonth = (k) => {
    const [yy, mm] = mk.split('-').map(Number);
    const d = new Date(yy, mm - 1 + k, 1);
    setMk(`${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`);
  };
  const openForm = (flow) => setForm({
    flow, category: flow === 'out' ? EXPENSE_CATS[0] : INCOME_CATS[1],
    amount: '', unit: '원', date: todayStr(), notes: '', indId: '',
  });
  const amountOf = (f) => {
    const n = parseFloat(String(f.amount).replace(/,/g, ''));
    if (!isFinite(n) || n < 0) return null;
    return Math.round(n * (f.unit === '만원' ? 10000 : 1));
  };
  const saveForm = () => {
    const won = amountOf(form);
    if (won === null) return showToast('⚠️ 금액을 숫자로 적어주세요');
    DB.addEvent({
      individualId: null, type: 'ledger', date: form.date || todayStr(),
      data: { flow: form.flow, category: form.category, amount: won,
              notes: (form.notes || '').trim(), indId: form.indId || null },
    });
    setForm(null); refresh();
    showToast(form.flow === 'out' ? `💸 지출 ${wonText(won)} 기록` : `💰 수입 ${wonText(won)} 기록`);
  };
  const Money = ({ v, color }) => <b style={{color, fontSize:15}}>{Number(v).toLocaleString()}</b>;

  return (
    <div style={{padding:'12px 16px 0'}}>
      {/* 달 고르기 */}
      <div style={{display:'flex', alignItems:'center', justifyContent:'center', gap:14, marginBottom:10}}>
        <button className="btn btn-secondary btn-sm" style={{width:'auto', padding:'5px 12px'}} onClick={() => shiftMonth(-1)}>‹</button>
        <span style={{fontSize:15, fontWeight:800}}>{mk.slice(0, 4)}년 {Number(mk.slice(5, 7))}월</span>
        <button className="btn btn-secondary btn-sm" style={{width:'auto', padding:'5px 12px'}}
          disabled={mk >= monthKeyOf(todayStr())} onClick={() => shiftMonth(1)}>›</button>
      </div>

      <div className="card" style={{margin:'0 0 12px'}}>
        <div style={{display:'flex', textAlign:'center'}}>
          <div style={{flex:1}}>
            <div style={{fontSize:11, color:'var(--text3)'}}>수입</div>
            <div style={{marginTop:3}}><Money v={m.income} color="#2E7D46" /><span style={{fontSize:11, color:'var(--text3)'}}>원</span></div>
          </div>
          <div style={{flex:1, borderLeft:'1px solid var(--border)', borderRight:'1px solid var(--border)'}}>
            ...
            <div style={{marginTop:3}}><Money v={m.expense} color="var(--danger)" /><span style={{fontSize:11, color:'var(--text3)'}}>원</span></div>
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:11, color:'var(--text3)'}}>남은 것</div>
            <div style={{marginTop:3}}><Money v={m.net} color={m.net >= 0 ? 'var(--accent2)' : 'var(--danger)'} /><span style={{fontSize:11, color:'var(--text3)'}}>원</span></div>
          </div>
        </div>
        <div style={{fontSize:11.5, color:'var(--text3)', textAlign:'center', marginTop:9, paddingTop:9, borderTop:'1px solid var(--border)'}}>
          {mk.slice(0, 4)}년 누계 · 수입 {y.income.toLocaleString()} · 지출 {y.expense.toLocaleString()} ·{' '}
          <b style={{color: y.net >= 0 ? 'var(--accent2)' : 'var(--danger)'}}>{y.net.toLocaleString()}원</b>
        </div>
      </div>

      {form ? (
        <div className="card" style={{margin:'0 0 12px'}}>
          <div style={{fontSize:13, fontWeight:800, marginBottom:9}}>{form.flow === 'out' ? '💸 지출 적기' : '💰 수입 적기'}</div>

          <div style={{fontSize:11, color:'var(--text3)', marginBottom:5}}>항목</div>
          <div style={{display:'flex', flexWrap:'wrap', gap:5, marginBottom:10}}>
            {(form.flow === 'out' ? EXPENSE_CATS : INCOME_CATS).map(c => (
              <button key={c} className="chip-btn" style={{fontSize:11.5, padding:'5px 10px', ...(form.category === c ? {background:'var(--accent-soft)', fontWeight:700} : {})}}
                onClick={() => setForm(f => ({ ...f, category: c }))}>{c}</button>
            ))}
          </div>

          <div style={{fontSize:11, color:'var(--text3)', marginBottom:4}}>금액</div>
          <div style={{display:'flex', gap:6}}>
            <input className="input" inputMode="decimal" placeholder="예) 30000" value={form.amount}
              onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} autoFocus />
            {['원','만원'].map(u => (
              <button key={u} className="chip-btn" style={{fontSize:12, padding:'6px 11px', flexShrink:0, ...(form.unit === u ? {background:'var(--accent-soft)', fontWeight:700} : {})}}
                onClick={() => setForm(f => ({ ...f, unit: u }))}>{u}</button>
            ))}
          </div>
          <div style={{fontSize:11.5, minHeight:17, margin:'5px 2px 8px', color: amountOf(form) ? 'var(--accent2)' : 'var(--text3)'}}>
            {form.amount ? (amountOf(form) !== null ? wonText(amountOf(form)) + ' 으로 적을게요' : '숫자로 적어주세요') : ' '}
          </div>

          <div style={{fontSize:11, color:'var(--text3)', marginBottom:4}}>날짜</div>
          <input className="input" type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} />

          <div style={{fontSize:11, color:'var(--text3)', margin:'10px 0 4px'}}>메모 <span style={{opacity:.65}}>(선택)</span></div>
          <input className="input" placeholder="예) 판게아 2봉" value={form.notes}
            onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />

          {form.flow === 'out' && (
            <>
              <div style={{fontSize:11, color:'var(--text3)', margin:'10px 0 4px'}}>
                어느 아이에게 쓴 돈인가요? <span style={{opacity:.65}}>(선택 · 개체 손익 계산에 씁니다)</span>
              </div>
              <select className="input" value={form.indId} onChange={e => setForm(f => ({ ...f, indId: e.target.value }))}>
                <option value="">— 특정 아이 아님 —</option>
                {individuals.filter(i => !i.isExternal).map(i => <option key={i.id} value={i.id}>{i.name}</option>)}
              </select>
            </>
          )}

          <div style={{display:'flex', gap:6, marginTop:13}}>
            <button className="btn btn-primary btn-sm" style={{flex:1}} onClick={saveForm}>저장</button>
            <button className="btn btn-secondary btn-sm" style={{flex:1}} onClick={() => setForm(null)}>취소</button>
          </div>
        </div>
      ) : (
        <div style={{display:'flex', gap:6, marginBottom:12}}>
          <button className="btn btn-primary btn-sm" style={{flex:1}} onClick={() => openForm('out')}>＋ 지출</button>
          <button className="btn btn-secondary btn-sm" style={{flex:1}} onClick={() => openForm('in')}>＋ 수입</button>
        </div>
      )}
      ...
      {monthRows.length === 0 ? (
        <div className="empty" style={{padding:'34px 20px'}}>
          <div className="empty-icon" style={{fontSize:36}}>💰</div>
          <p>이 달에는 오간 돈이 없어요.{'\n'}분양가를 적으면 수입은 저절로 올라옵니다.</p>
        </div>
      ) : monthRows.map(r => {
        const isIn = r.flow === 'in';
        const ind = r.indId ? byId[r.indId] : null;
        return (
          <div key={r.id} className="card" style={{margin:'0 0 8px', padding:'11px 13px'}}>
            <div style={{display:'flex', alignItems:'center', gap:9}}>
              <span style={{fontSize:17}}>{isIn ? '💰' : '💸'}</span>
              <div style={{flex:1, minWidth:0}}>
                <div style={{fontSize:13.5, fontWeight:700, display:'flex', alignItems:'center', gap:6, flexWrap:'wrap'}}>
                  <span>{r.category}</span>
                  {r.auto && <span className="badge" style={{fontSize:9.5, background:'var(--bg3)', color:'var(--text3)', border:'1px solid var(--border)'}}>자동</span>}
                </div>
                <div style={{fontSize:11.5, color:'var(--text3)', marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                  {fmtDate(r.date)}{r.label ? ' · ' + r.label : ''}{ind && !r.auto ? ' · ' + ind.name : ''}
                </div>
              </div>
              <div style={{fontSize:13.5, fontWeight:800, color: isIn ? '#2E7D46' : 'var(--danger)', flexShrink:0}}>
                {isIn ? '+' : '−'}{Number(r.amount).toLocaleString()}
              </div>
              {!r.auto && (
                <button onClick={() => setConfirmDel(confirmDel === r.id ? null : r.id)}
                  style={{background:'none', border:'none', cursor:'pointer', fontSize:13, color:'var(--text3)', flexShrink:0}}>🗑️</button>
              )}
              {r.auto && ind && (
            ...
            {confirmDel === r.id && (
              <div style={{marginTop:8}}>
                <div style={{fontSize:12, color:'var(--danger)', marginBottom:6}}>이 기록을 지울까요?</div>
                <div style={{display:'flex', gap:6}}>
                  <button className="btn btn-danger btn-sm" style={{flex:1}}
                    onClick={() => { DB.deleteEvent(r.ref); setConfirmDel(null); refresh(); showToast('가계부에서 지웠어요'); }}>삭제</button>
                  <button className="btn btn-secondary btn-sm" style={{flex:1}} onClick={() => setConfirmDel(null)}>취소</button>
                </div>
              </div>
            )}
          </div>
      ...
      {/* ── 전체 기간 합계 — 한눈에 얼마 벌고 얼마 썼는지 ── */}
      <div className="card" style={{margin:'14px 0 0', background:'var(--accent-soft)', border:'1px solid var(--accent-edge)'}}>
        <div style={{fontSize:12.5, fontWeight:800, color:'var(--accent2)', marginBottom:2}}>📊 전체 합계</div>
        <div style={{fontSize:11, color:'var(--text3)', marginBottom:10}}>
          {rows.length ? `기록 ${all.count}건${firstDate ? ` · ${fmtDate(firstDate)} ~ ${fmtDate(lastDate)}` : ''}` : '아직 기록이 없어요'}
          {op.count > 0 && ` · 이전 기록 ${op.count}건 포함`}
        </div>
        <div className="confirm-row">
          <span className="confirm-key">총 수입</span>
          <span className="confirm-val" style={{color:'#2E7D46', fontWeight:800}}>{all.income.toLocaleString()}원</span>
        </div>
        <div className="confirm-row">
          <span className="confirm-key">총 지출</span>
          <span className="confirm-val" style={{color:'var(--danger)', fontWeight:800}}>{all.expense.toLocaleString()}원</span>
        </div>
        <div className="confirm-row">
          <span className="confirm-key" style={{fontWeight:800}}>순익</span>
          <span className="confirm-val" style={{fontSize:16, fontWeight:800, color: all.net >= 0 ? 'var(--accent2)' : 'var(--danger)'}}>
            {all.net >= 0 ? '+' : ''}{all.net.toLocaleString()}원
          </span>
        </div>

        {/* 엑셀에서 가져온 입양가 — 날짜가 없어 달·해 칸에는 안 넣고 여기에 모아둡니다 */}
        {op.count > 0 && (
          <div style={{marginTop:10, paddingTop:10, borderTop:'1px solid var(--accent-edge)'}}>
            <button onClick={() => setOpenShow(v => !v)}
              style={{background:'none', border:'none', padding:0, cursor:'pointer', fontSize:11.5,
                      color:'var(--accent2)', fontWeight:700, textAlign:'left', width:'100%'}}>
              📦 이전 기록 · 입양가 {op.count}건 {op.expense.toLocaleString()}원 {openShow ? '▲' : '▼'}
            </button>
            <div style={{fontSize:11, color:'var(--text3)', marginTop:5, lineHeight:1.5}}>
              엑셀에서 가져온 아이들의 입양가예요. 산 날짜가 없어서 달·해 칸에는 넣지 않고 여기서만 셉니다.
            </div>
            {openShow && (
              <div style={{marginTop:8}}>
                {openRows.map(r => (
                  <div key={r.id} style={{display:'flex', alignItems:'center', gap:8, padding:'6px 0',
                    ...
                    <span style={{color:'var(--danger)', fontWeight:700, flexShrink:0}}>−{r.amount.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div style={{fontSize:11, color:'var(--text3)', lineHeight:1.6, margin:'12px 2px 0'}}>
        · 분양 수입은 <b>분양가를 적는 순간</b> 여기에 저절로 올라옵니다. 따로 또 적지 않으셔도 됩니다.<br />
        · 개체 구입비를 그 아이에게 연결해 두면, 프로필에서 <b>그 아이의 손익</b>이 보입니다.<br />
        · 대화로 "사료 3만원 샀어"처럼 말씀하셔도 여기에 들어옵니다.<br />
        · 메모에 <b>입양가 30만원</b>처럼 적힌 아이는 <b>개체 구입 지출</b>로 저절로 잡힙니다.
      </div>
      <div style={{height:16}} />
    </div>
  );
...
function LedgerScreen({ individuals, navigate, showToast, refreshIndividuals }) {
  const [seg, setSeg] = useState('sale');
  return (
    <div className="screen">
      <div className="header">
        <div className="header-row">
      ...
      <div style={{display:'flex', gap:6, padding:'12px 16px 0'}}>
        {[['sale','🏷️ 분양'],['money','💰 수입지출']].map(([k, l]) => (
          <button key={k} className="chip-btn" style={{flex:1, fontSize:13, padding:'8px 0', textAlign:'center',
            ...(seg === k ? {background:'var(--accent-soft)', fontWeight:700} : {})}}
            onClick={() => setSeg(k)}>{l}</button>
        ))}
      </div>

      {seg === 'sale'
        ? <SaleSection individuals={individuals} navigate={navigate} showToast={showToast} refreshIndividuals={refreshIndividuals} />
        : <MoneySection individuals={individuals} navigate={navigate} showToast={showToast} refreshIndividuals={refreshIndividuals} />}
...
function SmartChatScreen({ navigate, showToast, refreshIndividuals, presetGecko }) {
  ...
  const applyRetarget = (ind, scope) => {
    ...
    bot(`알겠어요, 방금 "${factLabel(moved)}" ${ind.name} 기록으로 옮겼어요 ✅\n계속 말씀하세요.`, [
      { label: '💾 이제 저장할래', kind: 'save' },
    ]);
  };
  ...
  const handleChip = (kind, value) => {
    if (kind === 'register-yes') {
      ...
      bot(`${g.name}, ${genderLabel(value)}으로 기억할게요.\n이제 편하게 이야기해주세요 🙂`);
    } else if (kind === 'save') {
      goConfirm();
      ...
      bot(`🦎 ${value.name}, 등록하고 파트너로 연결해뒀어요!\n저장하면 혈통에도 반영돼요.`);
         ...
         · 질문 답변(3.45)보다 앞      — "누구랑 붙일까?"의 "누구"를 새 이름으로 오해하지 않게 */
    {
      ...
      if (twoOrMore.length >= 2 && /(사이|관계|촌수|혈연|근친)/.test(text)) {
        ...
        bot(r ? mateWarning(twoOrMore[0], twoOrMore[1]).text
              : `${twoOrMore[0].name}랑 ${twoOrMore[1].name}는 기록상 혈연이 겹치지 않아요.\n붙이셔도 괜찮습니다 🙂`);
        return;
      }
    }
    ...
    if (!facts.length) {
      ...
      if (leftover.length < 2) {
        ...
        if (morphLine) {
          ...
          // 사용법 예시는 대화 첫 인사에서 한 번만 보여드립니다 (매번 나오면 길어서 읽히지 않습니다)
          bot(`${igunyo(target.name)}! 무슨 이야기인가요? 🦎`);
        }
        return;
      }
  ...
  const doSave = () => {
    ...
    let babyMsg = '';
    list.forEach(f => {
      // 가계부는 개체가 없는 기록입니다 (individualId 를 비워 개체 타임라인에 안 섞이게)
      if (f.type === 'ledger') {
      ...
      if (f.type === 'laying' && f.data && f.data.eggLook) {
        ...
        if (lay) DB.setEggUnits(lay.id, eggUnitsOf(lay, {}).map(u => ({ ...u, look: f.data.eggLook })));
      }
  ...
  const retargetTo = (f, indId) => {
    ...
    setRetarget(null);
    showToast(`🦎 ${euroWord(ind.name)} 옮겼어요`);
  };
    ...
    <div style={{display:'flex', flexDirection:'column', flex:1, minHeight:0}}>
      ...
      {phase === 'confirm' && (
        <div style={{padding:16, flex:1, overflowY:'auto'}}>
                  ...
                  <div key={i}>
                    <div className="confirm-row">
                      ...
                      <span className="confirm-val">
                        {fmtDate(f.date)}
                        {f.targetId && <button title="주인 바꾸기" onClick={() => setRetarget(retarget === f ? null : f)}
          ...
          {/* 산란을 적었으면 알 겉보기 상태를 여기서 함께 남깁니다 (불빛 비춰 보고 바로 적을 수 있게) */}
          {pending.filter(f => f.type === 'laying').map((f, i) => (
            <div key={'lk' + i} style={{margin:'0 4px 14px'}}>
              <div style={{fontSize:12, color:'var(--text2)', marginBottom:6}}>🥚 알 상태는 어떤가요? <span style={{color:'var(--text3)'}}>(선택)</span></div>
              <div style={{display:'flex', flexWrap:'wrap', gap:5}}>
                {EGG_LOOKS.map(L => (
                  <button key={L} className="chip-btn" style={{fontSize:11.5, padding:'5px 10px', ...((f.data.eggLook || '') === L ? {background:'var(--accent-soft)', fontWeight:700} : {})}}
                    onClick={() => updatePending(p => p.map(x => x === f ? { ...x, data: { ...x.data, eggLook: (x.data.eggLook === L ? '' : L) } } : x))}>{L}</button>
                ))}
              </div>
            </div>
          ))}

          {/* 부화를 적었으면 모프를 함께 물어봅니다 — 모르면 비워두면 됩니다 */}
          {pending.filter(f => f.type === 'hatching').map((f, i) => (
            <div key={'hm' + i} style={{margin:'0 4px 14px'}}>
              <div style={{fontSize:12, color:'var(--text2)', marginBottom:6}}>🐣 나온 아이들 모프는요? <span style={{color:'var(--text3)'}}>(모르면 비워두세요)</span></div>
              <input className="input" placeholder="예) 릴리화이트 · 할리퀸" value={f.data.morph || ''}
                onChange={e => { const v = e.target.value; updatePending(p => p.map(x => x === f ? { ...x, data: { ...x.data, morph: v } } : x)); }} />
              <div style={{fontSize:11, color:'var(--text3)', marginTop:5}}>
                비워두면 <b>아직 모르겠어요</b>로 남고, 홈 목록에는 모프 줄이 뜨지 않습니다.
              </div>
            </div>
          ))}
  ...
  return (
    <div className="screen">
            ...
            <div style={{flex:1}}>
              <div style={{display:'flex', alignItems:'center', gap:8, flexWrap:'wrap'}}>
                ...
                <button className="badge badge-gold"
                  style={{cursor:'pointer', opacity: isMine(gecko) ? 1 : .4,
                          filter: isMine(gecko) ? 'none' : 'grayscale(1)'}}
                  onClick={() => {
                    const on = isMine(gecko);
                    ...
                    showToast(on ? 'MY 표시를 뗐어요' : '🏷️ MY로 표시했어요 (우리 집에서 나온 아이)');
                  }}>
                  {isMine(gecko) ? 'MY' : 'MY 아님'}
                </button>
              </div>
      ...
      <div style={{padding:'0 16px 10px'}}>
        {healthOpen ? (
          <div className="card" style={{margin:0}}>
            <div style={{fontSize:13, fontWeight:800, marginBottom:8}}>⚠️ 어떤 이상이 있나요?</div>
            <div style={{display:'flex', flexWrap:'wrap', gap:5, marginBottom:10}}>
              {HEALTH_ISSUES.map(it => (
                <button key={it} className="chip-btn" style={{fontSize:12, padding:'6px 11px', ...(hIssue === it ? {background:'var(--accent-soft)', fontWeight:700} : {})}}
                  onClick={() => setHIssue(it)}>{it}</button>
              ))}
            </div>
            <div style={{fontSize:11, color:'var(--text3)', marginBottom:4}}>언제</div>
            <input className="input" type="date" value={hWhen} max={todayStr()} onChange={e => setHWhen(e.target.value)} />
            <div style={{fontSize:11, color:'var(--text3)', margin:'10px 0 4px'}}>메모 <span style={{opacity:.65}}>(선택)</span></div>
            <input className="input" placeholder="예) 사흘째 안 먹음 · 병원 다녀옴" value={hNote} onChange={e => setHNote(e.target.value)} />
            <div style={{display:'flex', gap:6, marginTop:12}}>
              <button className="btn btn-primary btn-sm" style={{flex:1}} onClick={() => {
                if (!hIssue) return showToast('⚠️ 어떤 이상인지 골라주세요');
                DB.addEvent({ individualId: gecko.id, type: 'health', date: hWhen || todayStr(),
                              data: { issue: hIssue, notes: hNote.trim() } });
                setHealthOpen(false); setHIssue(''); setHNote(''); setHWhen(todayStr());
                refreshLocal(); showToast('⚠️ 이상 기록을 남겼어요');
              }}>기록하기</button>
              <button className="btn btn-secondary btn-sm" style={{flex:1}} onClick={() => setHealthOpen(false)}>취소</button>
            </div>
          </div>
        ) : (
          <button className="btn btn-secondary" onClick={() => { setHealthOpen(true); setHWhen(todayStr()); }}>
            ⚠️ 이상이 있어요
          </button>
        )}
      </div>
      ...
      <div style={{padding:'0 16px 4px'}}>
        <div style={{fontSize:11, color:'var(--text3)', marginBottom:4}}>
          분양 상태{salePriceLabel(gecko) ? ` · 분양가 ${salePriceLabel(gecko)}` : ''}
          {gecko.status === 'sold' && !salePriceLabel(gecko) && (
        ...
        {(() => {
          ...
          return (
            <div style={{fontSize:11, color:'var(--text3)', marginBottom:6}}>
              {pr.cost ? `들인 돈 ${wonText(pr.cost)}` : '들인 돈 없음'}
              {pr.sold ? ` · 받은 돈 ${salePriceLabel(gecko) || '미기록'}` : ''}
              {pr.sold && (pr.got || pr.cost) ? (
                <b style={{color: pr.net >= 0 ? 'var(--accent2)' : 'var(--danger)'}}>
                  {` · 손익 ${pr.net >= 0 ? '+' : ''}${Number(pr.net).toLocaleString()}원`}
                </b>
              ) : ''}
            </div>
          );
...
function CalendarScreen({ navigate, individuals, showToast, onRemindersChanged }) {
  ...
  /* 먹인 날은 먹인 종류 그대로 한 글자만 — 예전엔 🍽️🥩 두 개가 붙어 눈에 거슬렸습니다 */
  const fedEmoji = (e) => ((e && e.data && e.data.foodType) === '슈푸' ? '🥣' : '🦗');
  const CALENDAR_FED_EMOJI = '🦗';
  ...
  return (
    <div className="screen">
      ...
      <div style={{padding:'12px 16px'}}>
        ...
        <div style={{fontSize:12, color:'var(--text3)', margin:'8px 4px 4px', lineHeight:1.7}}>
          💞 메이팅 · 🥚 산란 · 🐣 해칭<br/>
          🦗 충식 먹인 날 · 🥣 슈푸 먹인 날 · {CALENDAR_FEED_PLAN_EMOJI} 먹이 예정(흐리게)<br/>
          날짜를 누르면 그날 일정을 보고 고칠 수 있어요<br/>
          메모·사진·몸무게는 각 개체 프로필에서 볼 수 있어요
        </div>
...
function FeedingScreen({ navigate, showToast, refreshIndividuals }) {
  const [ver, setVer] = useState(0);
  const [search, setSearch] = useState('');            // 53마리 중 한 마리 찾기
  ...
  const allAte = () => {
    const additions = fp.inds
      .filter(i => !todayBy[i.id])
      .map(i => ({ individualId: i.id, type: 'feeding', date: today, data: { ate: true, foodType } }));
    DB.addEvents(additions);
    ...
    <div className="screen">
        ...
        <div className="card" style={{margin:'0 0 12px'}}>
          ...
          {search && (
            <div style={{fontSize:11.5, color:'var(--text3)', marginTop:6, display:'flex', alignItems:'center', gap:8}}>
              <span>"{search}" · {shown.length}마리</span>
              <button onClick={() => setSearch('')}
                style={{background:'none', border:'none', padding:0, cursor:'pointer', fontSize:11.5, color:'var(--accent2)', fontWeight:700}}>
                전체 보기
              </button>
            </div>
          )}
      ...
      <div style={{padding:'0 16px 20px'}}>
        ...
        <div style={{fontSize:11, color:'var(--text3)', textAlign:'center'}}>
          {feedWarn()}일 넘게 안 먹은 아이는 브리핑에서 따로 알려드려요<br/>
          (설정에서 조절할 수 있어요)
        </div>
      </div>
    </div>
...
function buildWorkbook(XLSX) {
  ...
  /* ── 5. 가계부 ── */
  const lrows = ledgerRows(DB.getEvents(), inds);
  const a7 = [['', '', '', '크레건설 가계부'], [], [],
    ['', '날짜', '구분', '항목', '내용', '개체', '금액(원)']];
  let inSum = 0, outSum = 0;
  [...lrows].reverse().forEach(r => {
    if (r.flow === 'in') inSum += r.amount; else outSum += r.amount;
    a7.push(['', r.date, r.flow === 'in' ? '수입' : '지출', r.category, r.label || '',
             r.indId ? nameOf(r.indId) : '', r.amount]);
  });
  a7.push([]);
  a7.push(['', '', '합계', '수입', inSum, '지출', outSum, '남은 것', inSum - outSum]);
  add('가계부', a7, [3, 12, 7, 14, 24, 12, 12]);
...
function SettingsScreen({ navigate, showToast, refreshIndividuals }) {
  ...
  const [linkEdit, setLinkEdit] = useState(false);     // 문의 링크 넣기 열림 여부
    ...
    <div className="screen">
      ...
      <div style={{padding:'16px', display:'flex', flexDirection:'column', gap:12}}>
        ...
        <div className="card" style={{margin:0}}>
          ...
          <div style={{fontSize:12, color:'var(--text3)', marginBottom:10, lineHeight:1.6}}>
            앱 전체 색을 바꿉니다.<br/>
            고른 색은 이 기기에 저장되고, 서버 동기화를 켜두셨다면 다른 기기에도 따라가요.
          </div>
          <div style={{display:'flex', flexDirection:'column', gap:8}}>
            {THEMES.map(t => {
        ...
        <div className="card" style={{margin:0}}>
          ...
          {/* 문의 창구 — 눌러서 바로 이동합니다 */}
          {contactLinks(settings).length > 0 && (
            <div style={{display:'flex', flexDirection:'column', gap:6, marginTop:12}}>
              {contactLinks(settings).map(l => (
          ...
          {linkEdit && (
            <div style={{marginTop:10}}>
              <div style={{fontSize:11, color:'var(--text3)', lineHeight:1.6, marginBottom:8}}>
                주소를 넣어두면 위에 버튼으로 뜹니다. 비워두면 그 줄은 안 보입니다.
              </div>
              {CONTACT_FIELDS.map(([k, label, ph]) => (
                <div key={k} style={{marginBottom:8}}>
                  <div style={{fontSize:11, color:'var(--text3)', marginBottom:4}}>{label}</div>
                  <input className="input" style={{padding:'9px 11px', fontSize:12.5}} placeholder={ph}
                    value={(settings.contact || {})[k] || ''}
                    onChange={e => {
                      const next = { ...settings, contact: { ...(settings.contact || {}), [k]: e.target.value } };
                      DB.saveSettings(next); setSettings(next);
                    }} />
                </div>
              ))}
              <div style={{fontSize:11, color:'var(--text3)'}}>적는 즉시 저장됩니다.</div>
            </div>
          )}
        </div>
      </div>
