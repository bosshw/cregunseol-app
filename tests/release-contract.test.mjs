...
test("keeps storage, synchronization, and core workflows intact", async () => {
  ...
  assert.match(source, /const APP_VERSION = '1\.9\.2'/);
...
test("keeps service worker and release metadata aligned", async () => {
  ...
  assert.equal(version.app, "1.9.2");
  assert.equal(version.schema, 1);
  assert.equal(version.minSchema, 1);
  assert.match(worker, /const CACHE = 'creg-v32'/);
...
test("keeps the v4.8 kinship, morph and voice contracts", async () => {
  ...
  // 버전 문자열 4곳
  assert.ok(source.includes("const APP_VERSION = '1.9.2'"), "APP_VERSION must be 1.9.2");
});
...
test("keeps the v5.0 particle, line-break and calendar fixes", async () => {
  ...
  assert.ok(source.includes("APP_VERSION = '1.9.2'"), "APP_VERSION must be 1.9.2");
...
test("keeps the v1.1 polish contracts", async () => {
  ...
  // ③ 승인된 브랜드 그림은 한 곳에서 관리합니다 (v1.3).
  assert.ok(source.includes("function BrandIcon"), "브랜드 그림 공통 컴포넌트");
  assert.ok(source.includes('name="gecko"'), "크한이 개체 그림");
  assert.ok(source.includes('name="egg"'), "알 그림");
  assert.doesNotMatch(source, /function pixelSvg|const DOT_GECKO_URI/, "이전 도트 생성기를 사용하지 않습니다");
...
/* ══════════════════════════════════════════
   v1.4 — 문의·건의 오픈채팅 창구
   ══════════════════════════════════════════ */
test("keeps the contact open-chat link", async () => {
  const raw = await readFile(file("src/app.jsx"), "utf8");
  const source = raw.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");

  /* 코드에 박아두면 앱을 쓰는 모든 분에게 똑같이 보입니다.
     ★ 주소가 비면 그 줄은 화면에서 사라집니다(contactLinks 가 걸러냅니다). */
  assert.match(source, /chat: 'https:\/\/open\.kakao\.com\/o\/sZ7wQNSd'/, "오픈채팅 주소가 있어야 합니다");
  assert.match(source, /\['chat',\s*'💬 문의·건의 오픈채팅'/, "무엇을 하는 곳인지 알 수 있는 이름");
  assert.match(source, /insta: 'https:\/\/www\.instagram\.com\/cre_construct'/, "인스타그램은 그대로");
});

test("keeps the v1.5 push-notification contracts", async () => {
  const source = await readFile(file("src/app.jsx"), "utf8");
  const worker = await readFile(file("sw.js"), "utf8");
  const sql = await readFile(file("supabase_push.sql"), "utf8");
  const fn = await readFile(file("supabase/functions/push/index.ts"), "utf8");

  // 무엇을 언제 보낼지는 앱의 pushPlan 한 곳에서만 정합니다
  assert.match(source, /function pushPlan\(/, "알림 일정표는 pushPlan 한 곳에서 만듭니다");
  assert.ok(!/clutchRows|feedSchedule/.test(fn),
    "부화·급여 계산이 서버로 새어 나가면 안 됩니다 — 규칙은 앱에만 둡니다");

  // 대표님이 고르신 두 가지만 보냅니다
  assert.match(source, /k: 'hatch'/, "부화 예정 알림");
  assert.match(source, /k: 'feed'/, "밥 주는 날 알림");
  assert.ok(!/k: 'nudge'|k: 'birthday'|k: 'ledger'/.test(source),
    "고르지 않으신 종류는 보내지 않습니다");
  assert.match(source, /const PUSH_AHEAD = 3;/, "부화는 사흘 전에 미리 한 번");

  // 아이폰은 홈 화면에 추가해야만 됩니다 — 이 안내가 빠지면 원인을 못 찾습니다
  assert.match(source, /'need-install'/, "아이폰 홈 화면 안내 상태가 있어야 합니다");
  assert.match(source, /standalone\(\)/, "홈 화면 여부를 실제로 봅니다");

  // 권한 요청은 버튼을 누른 그 순간에만
  assert.match(source, /Notification\.requestPermission\(\)/);
  assert.ok(!/useEffect\([^)]*Notification\.requestPermission/.test(source),
    "앱을 켜자마자 권한을 묻지 않습니다");

  // 알림 실패가 기록 동기화를 망치면 안 됩니다
  assert.match(source, /try \{ await PUSH\.syncPlan\(\); \} catch \(e\) \{\}/,
    "일정표 올리기는 실패해도 동기화를 세우지 않습니다");

  // 서비스워커가 받아서 띄우고, 눌렀을 때 갈 곳이 있어야 합니다
  assert.match(worker, /addEventListener\('push'/);
  assert.match(worker, /addEventListener\('notificationclick'/);
  assert.match(worker, /showNotification/);

  // 서버 쪽 안전장치
  assert.match(fn, /x-cron-key/, "새벽 발송은 암호로 잠급니다");
  assert.match(fn, /r\.gone/, "지워진 기기는 목록에서 정리합니다");
...
test("keeps a way to move a mis-linked hatching", async () => {
  const source = await readFile(file("src/app.jsx"), "utf8");

  /* 잘못 붙는 일은 반드시 생깁니다. 앱 안에 고칠 길이 없으면 기록이 영영 틀어집니다. */
  assert.match(source, /function moveHatchTo\(hatchId, toLayingId\)/,
    "부화 기록을 다른 알둥지로 옮기는 길이 있어야 합니다");
  assert.match(source, /data-testid="hatch-links"/, "알둥지 화면에 붙어 있는 부화 기록이 보여야 합니다");
  assert.match(source, /'move-to-' \+ r\.e\.id/, "옮길 곳을 고를 수 있어야 합니다");

  // 옮길 때 알 표시도 같이 맞춰야 합니다 — 한쪽만 고치면 화면이 어긋납니다
  assert.match(source, /u\.status === 'hatched' && u\.date === h\.date/, "떠나는 알은 대기로 되돌립니다");
...
test("counts visits without the owner and keeps the update bar to one line", async () => {
  const source = await readFile(file("src/app.jsx"), "utf8");
  // 대표님 계정 ID — 서버 auth.users 실제 값과 대조한 것 (2026-09-25). 한 글자라도 틀리면 본인 방문이 섞입니다
  assert.match(source, /const OWNER_UID = 'fa66ca69-3637-47b9-9cd3-58eac56c91d1'/);
  assert.match(source, /<span>새로운 버전이 올라왔습니다<\/span>/);
  assert.doesNotMatch(source, /VER\.note\(\) \?/, "새 버전 안내에 바꾼 내용을 적지 않습니다");
  const version = JSON.parse(await readFile(file("version.json"), "utf8"));
  assert.equal(version.note, "", "옛 버전 앱은 note 를 그대로 보여주므로 비워 둡니다");
});
